...
import { IsochroneAppUIResource } from '../../../src/resources/ui-apps/IsochroneAppUIResource.js';
...
describe('IsochroneAppUIResource', () => {
  beforeEach(() => {
  ...
  it('serves HTML with the mcp-app mime type and the public token baked in', async () => {
    ...
    const resource = new IsochroneAppUIResource({ httpRequest });

    const result = await resource.read('ui://mapbox/isochrone-app/index.html', {
    ...
    expect(entry.uri).toBe('ui://mapbox/isochrone-app/index.html');
    ...
    // Fill-layer-specific bits — not present in the directions template
    expect(entry.text as string).toContain('fill-color');
