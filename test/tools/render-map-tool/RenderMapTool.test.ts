...
describe('RenderMapTool', () => {
  ...
  it('passes through a map_matching self-fetch ref with no owner and no prior server-side state (simulated restart)', async () => {
    const { buildSelfFetchRef } =
      await import('../../../src/utils/selfFetchRef.js');
    const ref = buildSelfFetchRef('map_matching', {
      coordinates: [
    ...
    expect(sc.mapboxRender?.selfFetch).toEqual([
      {
        tool: 'map_matching',
        params: expect.objectContaining({ profile: 'driving' })
      }
    ]);
    const text = (result.content[0] as { type: 'text'; text: string }).text;
    expect(text).toContain('will also fetch and draw');
  ...
  it('passes through a category_search self-fetch ref with no owner and no prior server-side state (simulated restart)', async () => {
    const { buildSelfFetchRef } =
      await import('../../../src/utils/selfFetchRef.js');
    ...
    expect(sc.mapboxRender?.selfFetch).toEqual([
      {
        ...
        params: expect.objectContaining({ category: 'cafe' })
      }
    ]);
    const text = (result.content[0] as { type: 'text'; text: string }).text;
    expect(text).toContain('will also fetch and draw');
  ...
  it('passes through an optimization self-fetch ref with no owner and no prior server-side state (simulated restart)', async () => {
    ...
    expect(sc.mapboxRender?.selfFetch).toEqual([
      {
        tool: 'optimization',
        params: expect.objectContaining({ profile: 'mapbox/driving' })
      }
    ]);
    const text = (result.content[0] as { type: 'text'; text: string }).text;
    expect(text).toContain('will also fetch and draw');
