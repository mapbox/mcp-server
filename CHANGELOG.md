...
- **`directions_tool` and `isochrone_tool`: large responses no longer return a `mapbox://temp/...` ref that only exists on one hosted ECS task.** Previously, when a response exceeded the inline size threshold, both tools stored the full payload in `temporaryResourceManager` (an in-process `Map` with a 30-minute TTL) and returned a `mapbox://temp/{id}` URI for the client to `resources/read` afterward. The hosted deployment is fully stateless — every request can land on a different one of several ECS tasks behind a load balancer with no session stickiness — so a follow-up read landing on a different task than the one that computed the route found nothing and returned "Resource not found or expired." This was most visible on long routes (e.g. London→Scotland with `geometries="geojson"`), where the cleaned response crosses the 50KB inline threshold; it looked like "the Directions API is still computing" or "requested too soon," but Directions is synchronous and had already finished — the real cause was which task happened to serve the second HTTP request. Both tools now encode the full response directly into a new self-describing `mapbox://inline-response/{tool}?data=...` URI instead, following the same zero-server-state pattern already used by `mapbox://compute/`, `mapbox://selffetch/`, and `mapbox://inline/payload`. Resolving the ref is a pure function of the URI string, so it works identically no matter which task (or process) handles the follow-up `resources/read` - verified with a real two-process integration test that computes the route in one spawned server, kills it, and resolves the ref from a second, independent process. This also drops the old per-ref owner check, matching the no-owner-check precedent already used by the other self-describing ref schemes. `static_map_image_tool`'s equivalent large-payload fallback still uses `temporaryResourceManager` and is intentionally left as-is for now (its stored payload is base64-encoded binary image data, not JSON, so it needs its own encoding scheme to avoid double-base64 overhead) - tracked as a fast follow-up.
