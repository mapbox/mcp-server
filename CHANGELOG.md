...
- **`render_map_tool`: an enriched place with no photo on file looked identical to a row that hadn't been enriched yet.** The panel's thumbnail only ever changed appearance when a photo was found; a `Place Details` response that came back with no `photos` left the plain numbered badge untouched, indistinguishable from "still waiting on the batch call" or "filtered out for an incompatible id" — losing the distinction the original mockup had. `enrichSidePanel` now switches a successfully-enriched-but-photo-less place to a dashed, unfilled look (`panel-thumb no-photo`) instead of leaving it as the default solid-orange numbered state. First attempt at this also cleared the number itself, matching the `has-photo` case — caught live: an empty box with nothing in it just reads as broken, not as an intentional "checked, found nothing" signal. Keeps the number visible; only the border/fill changes.
