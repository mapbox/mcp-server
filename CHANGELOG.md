...
- **`static_map_image_tool`: large images no longer return a `mapbox://temp/...` ref that only exists on one hosted ECS task.** Same underlying bug as `directions_tool`/`isochrone_tool` (see the fast-follow entry above, or PR #257): images over the 700KB inline threshold were stashed in `temporaryResourceManager` (an in-process `Map`, 30-minute TTL) behind a `mapbox://temp/static-map-{id}` ref, which a follow-up `resources/read` landing on a different hosted ECS task couldn't see. Directly reusing the JSON-ref approach from that fix doesn't work here: base64-encoding raw image bytes into a URI blows past the MCP SDK's own hard 1,000,000-character resource-URI cap almost immediately (confirmed live — a 750KB image alone encodes to a 1,024,055-character URI), and images needing this fallback can run into multiple megabytes (e.g. 1280x1280 @2x). Instead, the new `mapbox://inline-image/static-map?data=...` ref encodes only the original request parameters (center, zoom, style, overlays, etc. — a few hundred bytes at most) and `resources/read` re-issues the Static Images API request at read time using those params and the reader's own resolved access token. The Static Images API is a deterministic renderer, so this reproduces the exact same image on demand rather than storing it, works from any task, and has no size ceiling — verified with a real two-process integration test that fetches the image in one spawned server, kills it, and re-fetches the identical bytes from a second, independent process. The one tradeoff: a follow-up read costs one extra Static Images API call instead of returning cached bytes.
