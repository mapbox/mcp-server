...
- **`render_map_tool`: an enriched place with no photo on file looked identical to a row that hadn't been enriched yet.** The panel's thumbnail only ever changed appearance when a photo was found; a `Place Details` response that came back with no `photos` left the plain numbered badge untouched, indistinguishable from "still waiting on the batch call" or "filtered out for an incompatible id" — losing the distinction the original mockup had (a dashed "no photo on file" placeholder, distinct from both a real photo and an unenriched row). `enrichSidePanel` now gives a successfully-enriched-but-photo-less place its own look (`panel-thumb no-photo`: dashed border, no fill, numbered badge cleared) instead of leaving it as the default numbered state.
