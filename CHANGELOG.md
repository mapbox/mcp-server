...
- **`render_map_tool`: side panel + Place Details enrichment for search/category-search results.** When a `search_and_geocode_tool`/`category_search_tool` result is rendered via `render_map_tool`'s self-fetch mechanism, the map iframe now also renders a Google-Maps-style results list (docked top-right, one row per pin: name, category, distance) alongside the existing numbered markers. Once that Search-Box-only list renders, the iframe fires a single batched `POST` to the Places API's `/details/retrieve` endpoint (up to 100 `mapbox_id`s per request) to backfill each row with a photo thumbnail (when available) and a popularity score, and appends the place's phone number onto its marker popup — all client-side, against the public token, same as every other self-fetch call. Confirmed live against the real API that the Places Details endpoint needs no secret scope (its docs list none, unlike write-oriented endpoints that do) and that the batch form works over a public token; also confirmed live that photo coverage is inconsistent (0 of 3 real POIs looked up during testing had one), so a missing thumbnail is the common case, not a bug. Mapbox has no star-rating field for POIs — `score.popularity` (0–1) is the closest analog and is what the panel shows instead. Enrichment failures (the Places API's 1,000 records/month Public Preview quota, a transient error) degrade silently; the already-rendered Search-Box-only list is left as-is. Clicking a panel row flies the map to that marker.

  Does not yet handle a single `render_map_tool` call whose `payload_refs` merge two POI-producing self-fetch refs together (e.g. two `category_search_tool` results onto one map) — the panel would show only the later ref's rows, though both sets of markers still render correctly. Left unhandled pending a real case, rather than building accumulation logic speculatively.

- **`render_map_tool`: the results panel above now also works on hand-composed inline markers, not just `payload_refs`.** Tested live in Claude Desktop against the self-fetch mechanism above and found that, even with `category_search_tool`'s output explicitly instructing the model to call `render_map_tool({ "payload_refs": [...] })`, Claude Desktop reliably built the map from its own inline `markers` instead — presumably because it already had the names/addresses/coordinates in hand from the search tool's text output. That means the self-fetch path the panel depended on was rarely, if ever, exercised in real usage. `markers[]` (both `MapAppPayload` and `render_map_tool`'s input schema) now accepts optional `id` (a POI's `mapbox_id`), `name`, `category`, and `distanceMeters` fields — set `id` on a hand-composed marker and it gets the exact same results panel and batched Place Details enrichment a self-fetch result gets, with no extra tool calls. `render_map_tool`'s own description and the `markers[].id` field's description now call this out explicitly, since the whole point is for the model to reach for it without being told to in the prompt. Internally, `mapAppHtml.ts`'s panel/enrichment trigger moved from being hardcoded into the two search self-fetch functions to a generic `maybeShowSidePanel()` called from both `renderBody()` (inline markers) and `mergeAdditionalPayload()` (self-fetch markers) — same code path either way. Re-tested live in Claude Desktop after this change with an ordinary prompt ("find coffee shops near Herndon, VA and show them on a map", no mention of details/photos/enrichment) — Claude populated `markers[].id` on its own and the panel + batched enrichment fired correctly, including the case where none of the 10 real results had a photo, confirming the coverage gap noted above is real-world common, not a corner case.

- **Added automated test coverage for the panel/enrichment feature above**, which previously had none. Required extending `mapAppHtml.script.test.ts`'s hand-rolled VM-sandbox fake DOM elements with `appendChild`/`removeChild`/`firstChild`/`addEventListener` (the panel code builds real nodes rather than `innerHTML` strings + `querySelector`, specifically so this sandbox — which has no HTML parser — could exercise it) and fixing a real gap in the fetch mock: it previously dropped a fetch call's second (`init`) argument entirely, which happened to never matter before because every prior self-fetch call was a plain `GET` — the Place Details batch call is the first `POST` with a JSON body, and the dropped argument would have made its shape unverifiable. New tests cover: `derivePanelItems`'s pure marker-filtering/numbering logic directly; the self-fetch path end-to-end (panel renders, then enriches with a photo/popularity score from a mocked batched call, click-to-flyTo, phone reaching the marker's popup); a negative case confirming non-POI markers (e.g. a directions route's start/end pins) never get a panel; enrichment failure leaving the initial rows intact; the inline-marker path end-to-end; and that a second render replaces rather than accumulates panel rows.

### Fixed

- **`render_map_tool`: a single OSM-sourced result in a category/search-box result set silently zeroed out Place Details enrichment for every result in that render, not just the bad one.** Caught while investigating why a real "coffee shops near Herndon, VA" render showed popularity scores but no photos for any of 10 results — re-ran the same kind of query directly against the live API and found 2 of 10 `mapbox_id`s decoded to `urn:mbxpoi-osm:n<osm-node-id>` (an OpenStreetMap-sourced id) rather than the Places API's native `urn:mbxpoi:<uuid>` scheme, and confirmed live that the batch `/details/retrieve` endpoint rejects the **entire** request with a 422 if even one id doesn't match its native scheme — the same 422 `PlaceDetailsTool.ts` already works around server-side for boundary ids, but the panel's batch call had no equivalent guard. `fetchPlaceDetailsBatch` now filters ids client-side (decode each with `atob` and check for the `urn:mbxpoi:` prefix) before sending, so a mix of native and OSM-sourced ids enriches the native ones instead of enriching none of them. Separately confirmed live that a fully successful batch can still have zero photo coverage regardless — traced this further and it's an account-wide entitlement gate, not per-place data sparsity: re-tested against Mapbox's own documented example POI (the one their Details API docs page uses to show 25 photos) with this project's account token and got zero photos, on both the new Places API and the legacy Details API identically. Not something a code change can address; noted for whoever manages this account's Mapbox relationship.

- **`render_map_tool`: an enriched place with no photo on file looked identical to a row that hadn't been enriched yet.** The panel's numbered thumbnail only ever changed appearance when a photo was found, so a `Place Details` response that came back with no `photos` left the plain numbered square untouched — indistinguishable from "still waiting on the batch call" or "filtered out for an incompatible id," and losing the distinction the original mockup had (photo, explicit "no photo on file" placeholder, and a visit-order number were all visually separate there). Split the merged thumbnail into two elements to match: a `.panel-badge` that always shows the visit-order number regardless of enrichment state, and a `.panel-thumb` that's now free to be a real photo, an empty placeholder (not yet enriched), or a dashed `no-photo` placeholder with its own "no photo" label (enriched, nothing found) — three visually distinct states instead of two elements fighting over one 28px square.

- **`render_map_tool`: the panel's numbers didn't correspond to anything on the map when a caller built plain, unlabeled markers.** Caught live: Claude built inline POI markers with no `style`/`label` at all for a "coffee shops near Herndon" render, so the map showed generic blue pins with nothing distinguishing #1 from #7 — the panel's own numbering (assigned positionally, independent of what's actually drawn) had nothing to visually correspond to. Rather than rely on a caller remembering to number its markers consistently with the panel — the same kind of prompt-compliance problem already solved for `payload_refs` earlier in this feature — `renderBody`/`mergeAdditionalPayload` now call a new `applyPanelNumbering()` before drawing markers: any marker with `id` set but no explicit `label` gets promoted to the numbered-badge style with a label matching its panel row, guaranteed by code rather than hoped for from the caller. A marker that already specifies its own `style`/`label` is left alone.

- **`ground_location_tool`'s self-fetched nearby-POI markers now get the same results panel + Place Details enrichment as `search_and_geocode_tool`/`category_search_tool`.** Verified this was the only other self-fetch path drawing real business/POI markers (directions, isochrone, map matching, and optimization's trip-stop markers are all route/geometry data, not searchable places, and were confirmed to never set an `id`) — but `ground_location`'s nearby-places query already runs a category search internally and simply wasn't threading `mapbox_id`/`poi_category`/`distance` through to its markers. Now does, via the same generic `id`/`name`/`category`/`distanceMeters` marker fields every other path uses, so it picks up the panel/enrichment/auto-numbering for free with no separate code path.
