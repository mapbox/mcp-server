...
export class TemporaryDataResource extends BaseResource {
  ...
  async read(
    ...
    // (request auth first, then the env token for stdio/single-user). A mismatch
    // returns the SAME "not found" response as a missing resource, so a caller
    // cannot probe whether someone else's resource exists.
