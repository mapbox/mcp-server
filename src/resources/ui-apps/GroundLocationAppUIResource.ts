...
/**
 * Serves HTML for the Ground Location App MCP App.
 *
 * Receives a reverse-geocoded origin and a list of nearby POIs from
 * `ground_location_app_tool`. Renders the origin as a labeled marker
 * and each POI as a numbered pin with a popup. The view fits all points.
 */
...
(function() {
  ...
  function handleToolResult(result) {
    ...
    if (!payload || !payload.origin) {
      loadingEl.style.display = 'none';
      errorEl.textContent = 'No location data in tool result.';
      errorEl.style.display = 'block';
      return;
    }
    if (payload.summary) {
  ...
  function extractPayload(result) {
    if (result && result.structuredContent && result.structuredContent.ground_location) {
      return result.structuredContent.ground_location;
    }
      ...
      for (var i = 0; i < result.content.length; i++) {
        ...
        if (c && c.type === 'text' && typeof c.text === 'string') {
          try {
            ...
            if (parsed && parsed.origin) return parsed;
            if (parsed && parsed.ground_location && parsed.ground_location.origin) {
              return parsed.ground_location;
            }
          } catch (_) { /* not JSON */ }
        }
      }
