...
const CategorySearchInputSchema = z.object({
  category: z
    .string()
    .describe(
      'The canonical category ID to search for. See all 90+ supported categories: read_resource("resource://mapbox-categories"). Common examples: restaurant, hotel, pharmacy, grocery, bank, cafe, school, hospital, shopping, food_and_drink'
    )
    .refine((val) => MAPBOX_SUPPORTED_CATEGORIES.includes(val as any), {
      message: `Invalid category. Must be one of the supported categories. Use read_resource("resource://mapbox-categories") to see the complete list.`
    }),
  language: z
    .string()
    .optional()
